...
public class FightClass {
	...
	public static long getNearestEnemy(EntityAI ai) throws LeekRunException {
		if (ai.getEntity().getCell() == null)
			return -1;
		var entities = ai.getState().getEnemiesEntities(ai.getEntity().getTeam());
		...
		for (var l : entities) {
			if (l.isDead() || l.getCell() == null)
				continue;
			int d = Map.getDistance2(ai.getEntity().getCell(), l.getCell());
			if (d < dist || dist == -1) {
				dist = d;
				nearest = l;
			}
	...
	public static LegacyArrayLeekValue getDeadEnemies_v1_3(EntityAI ai) throws LeekRunException {
		var result = new LegacyArrayLeekValue(ai);
		for (var e : ai.getState().getAllEntities(true)) {
	...
	public static LegacyArrayLeekValue getEnemies_v1_3(EntityAI ai) throws LeekRunException {
		var result = new LegacyArrayLeekValue(ai);
		for (var e : ai.getState().getEnemiesEntities(ai.getEntity().getTeam(), true)) {
			result.push(ai, (long) e.getFId());
		}
		return result;
	}
	...
	public static long getEnemiesLife(EntityAI ai) {
		int life = 0;
		for (var e : ai.getState().getEnemiesEntities(ai.getEntity().getTeam())) {
			life += e.getLife();
		}
		return life;
	}
	...
	public static long getEnemiesCount(EntityAI ai) throws LeekRunException {
		return ai.getState().getEnemiesEntities(ai.getEntity().getTeam(), true).size();
	}

	public static Long getAlliedTurret(EntityAI ai) {
		if (ai.getState().getType() == Fight.TYPE_TEAM) {
			for (var e : ai.getState().getTeamEntities(ai.getEntity().getTeam(), true)) {
	...
	public static Long getEnemyTurret(EntityAI ai) {
		if (ai.getState().getType() == Fight.TYPE_TEAM) {
			for (var e : ai.getState().getEnemiesEntities(ai.getEntity().getTeam(), true)) {
				if (e.getType() == Entity.TYPE_TURRET) return (long) e.getFId();
			}
	...
	public static LegacyArrayLeekValue getAliveAllies_v1_3(EntityAI ai) throws LeekRunException {
		var retour = new LegacyArrayLeekValue(ai);
		for (var l : ai.getState().getTeamEntities(ai.getEntity().getTeam())) {
	...
	public static long getAliveAlliesCount(EntityAI ai) throws LeekRunException {
		return ai.getState().getTeamEntities(ai.getEntity().getTeam()).size();
	}

	public static LegacyArrayLeekValue getDeadAllies_v1_3(EntityAI ai) throws LeekRunException {
		var retour = new LegacyArrayLeekValue(ai);
		for (var l : ai.getState().getTeamEntities(ai.getEntity().getTeam(), true)) {
			if (l.isDead()) {
				retour.push(ai, (long) l.getFId());
			}
		}
	...
	public static long getAlliesCount(EntityAI ai) throws LeekRunException {
		return ai.getState().getTeamEntities(ai.getEntity().getTeam(), true).size();
	}
	...
	public static long getCellToUseWeapon(EntityAI ai, long value1, Object value2, Object value3) throws LeekRunException {
		...
		if (target != null && target.getCell() != null && weapon != null) {
			...
			var possible = ai.getState().getMap().getPossibleCastCellsForTarget(weapon.getAttack(), target.getCell(), cells_to_ignore);
			if (possible != null && possible.size() > 0) {
				if (possible.contains(ai.getEntity().getCell())) {
					cell = ai.getEntity().getCell().getId();
				} else {
					List<Cell> path = ai.getState().getMap().getAStarPath(ai.getEntity().getCell(), possible, cells_to_ignore);
					if (path != null) {
						if (path.size() > 0)
	...
	public static long getCellToUseWeaponOnCell(EntityAI ai, long value1, Object value2, Object value3) throws LeekRunException {
		...
		if (target != null && weapon != null) {
			...
			var possible = ai.getState().getMap().getPossibleCastCellsForTarget(weapon.getAttack(), target, cells_to_ignore);
			if (possible != null && possible.size() > 0) {
				if (possible.contains(ai.getEntity().getCell())) {
	...
	public static long getCellToUseChip(EntityAI ai, Object chip, Object t, Object value3) throws LeekRunException {
		...
		var target = ai.getFight().getEntity(ai.integer(t));
		int cell = -1;
		if (target == null)
			return cell;
		...
		if (value3 instanceof GenericArrayLeekValue) {
			...
			cells_to_ignore.add(ai.getEntity().getCell());
		List<Cell> possible = ai.getState().getMap().getPossibleCastCellsForTarget(template.getAttack(), target.getCell(), cells_to_ignore);
		if (possible != null && possible.size() > 0) {
			if (possible.contains(ai.getEntity().getCell())) {
				cell = ai.getEntity().getCell().getId();
			} else {
	...
	public static long getCellToUseChipOnCell(EntityAI ai, Object chip, Object cell, Object value3) throws LeekRunException {
		...
		List<Cell> possible = ai.getState().getMap().getPossibleCastCellsForTarget(template.getAttack(), target, cells_to_ignore);
		if (possible != null && possible.size() > 0) {
			if (possible.contains(ai.getEntity().getCell())) {
				retour = ai.getEntity().getCell().getId();
			} else {
	...
	public static long moveTowardLeeks(EntityAI ai, GenericArrayLeekValue leeks, long pm_to_use) throws LeekRunException {
		...
		if (pm > 0) {
			...
			for (int i = 0; i < leeks.size(); i++) {
				var value = ai.integer(leeks.get(i));
				var l = ai.getFight().getEntity(value);
				if (l != null && !l.isDead())
					targets.add(l.getCell());
			}
			if (targets.size() != 0) {
				List<Cell> path = ai.getState().getMap().getAStarPath(ai.getEntity().getCell(), targets);
				if (path != null) {
					used_pm = ai.getState().moveEntity(ai.getEntity(), path.subList(0, Math.min(pm, path.size())));
				}
			}
		}
		return used_pm;
	...
	public static long moveTowardCells(EntityAI ai, GenericArrayLeekValue cells, long pm_to_use) throws LeekRunException {
		...
		if (pm > 0) {
			...
			for (int i = 0; i < cells.size(); i++) {
				var value = ai.integer(cells.get(i));
				Cell c = ai.getState().getMap().getCell(value);
				if (c != null)
					targets.add(c);
			}
			if (targets.size() != 0) {
	...
	public static long moveAwayFrom(EntityAI ai, long leek_id, long pm_to_use) throws LeekRunException {
		...
		if (pm > 0) {
			...
			if (target != null && target.getCell() != null) {
				...
				List<Cell> path = ai.getState().getMap().getPathAway(ai.getEntity().getCell(), cells, pm);
				if (path != null) {
	...
	public static long moveAwayFromEntities(EntityAI ai, GenericArrayLeekValue leeks, long pm_to_use) throws LeekRunException {
		...
		if (pm > 0) {
			...
			if (targets.size() != 0) {
				List<Cell> path = ai.getState().getMap().getPathAway(ai.getEntity().getCell(), targets, pm);
				if (path != null) {
					used_pm = ai.getState().moveEntity(ai.getEntity(), path);
				}
			}
	...
	public static long moveAwayFromLine(EntityAI ai, long cell1, long cell2, long pm_to_use) throws LeekRunException {
		...
		if (pm > 0) {
			...
			Cell target = ai.getState().getMap().getCell((int) cell1);
			Cell target2 = ai.getState().getMap().getCell((int) cell2);
			// Si il est trouvé on calcule le path
			if (target != null && target2 != null) {
				List<Cell> path = ai.getState().getMap().getPathAwayFromLine(ai.getEntity().getCell(), target, target2, pm);
	...
	public static long moveTowardLine(EntityAI ai, long cell1, long cell2, long pm_to_use) throws LeekRunException {
		...
		if (pm > 0) {
			...
			if (target != null && target2 != null && ai.getEntity().getCell() != null) {
				List<Cell> path = ai.getState().getMap().getPathTowardLine(ai.getEntity().getCell(), target, target2);
				// Si un path a été trouvé on y va
				if (path != null) {
					used_pm = ai.getState().moveEntity(ai.getEntity(), path.size() > pm ? path.subList(0, pm) : path);
				}
			}
		}
		return used_pm;
	...
	public static LegacyArrayLeekValue getCellsToUseWeapon_v1_3(EntityAI ai, long value1, Object value2, Object value3) throws LeekRunException {
		...
		if (value3 instanceof GenericArrayLeekValue) {
			...
			cells_to_ignore.add(ai.getEntity().getCell());
		List<Cell> possible = ai.getState().getMap().getPossibleCastCellsForTarget(weapon.getAttack(), target.getCell(), cells_to_ignore);
	...
	public static LegacyArrayLeekValue getCellsToUseWeaponOnCell_v1_3(EntityAI ai, long value1, Object value2, Object value3) throws LeekRunException {
		...
		if (value3 instanceof GenericArrayLeekValue) {
			...
			cells_to_ignore.add(ai.getEntity().getCell());
		List<Cell> possible = ai.getState().getMap().getPossibleCastCellsForTarget(weapon.getAttack(), target, cells_to_ignore);
	...
	public static LegacyArrayLeekValue getCellsToUseChip_v1_3(EntityAI ai, long chip_id, long target_leek_id, Object value3) throws LeekRunException {
		...
		var target = ai.getFight().getEntity(ai.integer(target_leek_id));
		// On récupère le sort
	...
	public static LegacyArrayLeekValue getCellsToUseChipOnCell_v1_3(EntityAI ai, long chip_id, long target_cell_id, Object value3) throws LeekRunException {
		...
		Cell target = ai.getState().getMap().getCell(ai.integer(target_cell_id));
		// On récupère le sort
		Chip template = Chips.getChip(ai.integer(chip_id));
	 ...
	 */
	public static Long getNearestEnemyTo(EntityAI ai, long leek_id) {
		...
		for (var l : entities) {
			...
			if (l.getCell() == null)
				continue;
			int d = Map.getDistance2(entity.getCell(), l.getCell());
			if (d < dist || dist == -1) {
				dist = d;
				nearest = l;
			}
	 ...
	 * @return Ennemi le plus proche
	 */
	public static Long getNearestEnemyToCell(EntityAI ai, long cell_id) {
		...
		for (var l : entities) {
			if (l.isDead() || l.getCell() == null)
				continue;
			int d = Map.getDistance2(cell, l.getCell());
			if (d < dist || dist == -1) {
				dist = d;
				nearest = l;
			}
